...
<div class="row jarak-form black-text">
    <form class="col s12" method="post" action="">
        <div class="input-field col s3">
            ...
            <input id="dari_tanggal" type="text" value="<?=(!empty($_REQUEST['dari_tanggal']))?$_REQUEST['dari_tanggal'] : date('Y-m-d')?>" name="dari_tanggal" id="dari_tanggal" required>
            <label for="dari_tanggal">Dari Tanggal</label>
        </div>
        <div class="input-field col s3">
            <i class="material-icons prefix md-prefix">date_range</i>
            <input id="sampai_tanggal" type="text" value="<?=(!empty($_REQUEST['sampai_tanggal']))?$_REQUEST['sampai_tanggal']:date('Y-m-d')?>" name="sampai_tanggal" id="sampai_tanggal" required>
...
if(isset($_REQUEST['submit'])){
    ...
    if($_REQUEST['dari_tanggal'] == "" || $_REQUEST['sampai_tanggal'] == ""){
        ...
        $query = $db->select('tbl_surat_masuk',"*",["tgl_diterima[<>]"=>[$dari_tanggal,$sampai_tanggal]]);

        echo '
            <div class="row agenda">
            <div class="disp hidd">';

            echo '<img class="logodisp" src="./upload/'.$instansi['logo'].'"/>';

            echo '<h6 class="up">'.$instansi['institusi'].'</h6>';

            echo '<h5 class="nama" id="nama">'.$instansi['nama'].'</h5><br/>';

            echo '<h6 class="status">'.$instansi['status'].'</h6>';

            echo '<span id="alamat">'.$instansi['alamat'].'</span>
                    ...
                    if(count($query) > 0){
                        ...
                        foreach($query as $row){
                            ...
                            <tr>
                                ...
                                $row['id_user'] = $db->get('tbl_user','nama',['id_user'=>$row['id_user']]);
